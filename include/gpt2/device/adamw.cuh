...
__device__ void adamw_kernel_device(float *params, const float *grads, float *m,
                                    float *v, int N, float learning_rate,
                                    float beta1, float beta2, float eps,
                                    float weight_decay, int t, int blockIdx_x) {
  ...
  // weight_decay * param)
  params[idx] = param - learning_rate * (m_hat / (sqrtf(v_hat) + eps) +
                                         weight_decay * param);
}
