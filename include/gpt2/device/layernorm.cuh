...
// gamma: [n_embd]
// beta: [n_embd]
__device__ void layernorm_forward_device(float *out, float *input,
                                         float *weight, float *bias,
                                         float *ln_mean, float *ln_rstd,
                                         int seq_len, int n_embd,
                                         int blockIdx_x) {
  int batch_idx = blockIdx_x;
  int seq_idx = threadIdx.x;

  // threadIdx.x indexes sequence positions (time steps), so compare against
  // seq_len
  if (seq_idx < seq_len) {
...
// weight: [n_embd]
__device__ void
layernorm_backward_device(float *g_input, float *g_weight, float *g_bias,
                          const float *g_out, const float *input,
                          const float *weight, const float *ln_mean,
                          const float *ln_rstd, int batch_size, int seq_len,
                          int n_embd, int blockIdx_x) {
  int batch_idx = blockIdx_x;
  int seq_idx = threadIdx.x;

  if (seq_idx < seq_len) {
    // Get pointers to the current batch and sequence position
    const float *g_out_bt =
        g_out + batch_idx * seq_len * n_embd + seq_idx * n_embd;
    const float *input_bt =
        input + batch_idx * seq_len * n_embd + seq_idx * n_embd;
    float *g_input_bt =
        g_input + batch_idx * seq_len * n_embd + seq_idx * n_embd;
    ...
    for (int i = 0; i < n_embd; i++) {
      ...
      // Gradient contribution to weight (atomic add for thread-safe
      // accumulation)
