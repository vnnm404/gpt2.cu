...
__device__ void attention_forward_device(float *out, float *preatt, float *att,
  ...
  for (int t2 = 0; t2 <= t; t2++) {
    const float *key_t2 =
        inp + b * T * C3 + t2 * C3 + h * hs + C; // +C for key offset
...
__device__ void attention_backward_device(float *g_inp, float *g_preatt,
                                          float *g_att, const float *g_out,
                                          const float *inp, const float *att,
  ...
  // Compute g_att[t, t2] = dot(value[t2], g_out[t]) - direct write, no atomic
  // needed
  for (int t2 = 0; t2 <= t; t2++) {
    float acc = 0.0f;
    for (int i = 0; i < hs; i++) {
      acc += inp[(b * T + t2) * C * 3 + h * hs + C * 2 + i] *
             g_out[(b * T + t) * C + h * hs + i];
    }
    g_att[(b * NH + h) * T * T + t * T + t2] = acc;
  }

  // Compute g_preatt using O(n) softmax backward - direct write, no atomic
  // needed
  float ds = 0.0f;
  for (int t2 = 0; t2 <= t; t2++) {
    ds += att[(b * NH + h) * T * T + t * T + t2] *
          g_att[(b * NH + h) * T * T + t * T + t2];
  }
  for (int t2 = 0; t2 <= t; t2++) {
    g_preatt[(b * NH + h) * T * T + t * T + t2] =
        att[(b * NH + h) * T * T + t * T + t2] *
        (g_att[(b * NH + h) * T * T + t * T + t2] - ds);
  }
  ...
  for (int t2 = 0; t2 <= t; t2++) {
    ...
    for (int i = 0; i < hs; i++) {
      atomicAdd(&g_inp[(b * T + t2) * C * 3 + h * hs + C + i],
                inp[(b * T + t) * C * 3 + h * hs + i] * g_preatt_val);
      atomicAdd(&g_inp[(b * T + t2) * C * 3 + h * hs + C * 2 + i],
                att_val * g_out[(b * T + t) * C + h * hs + i]);
    }
  }
}
