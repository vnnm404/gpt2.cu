#ifndef GPT2_LAYERS_RESIDUAL_H
#define GPT2_LAYERS_RESIDUAL_H

#include <cuda_runtime.h>

/* Residual layer structures and functions will be defined here */

__global__ void residual_forward(float *out, const float *input, const float *residual, int batch_size, int seq_len, int n_embd);
__global__ void residual_backward(float *g_inp1, float *g_inp2, const float *g_out, int N);

__device__ void residual_forward_device(float *out, const float *input, const float *residual, int batch_size, int seq_len, int n_embd,
                                        int blockIdx_x);
__device__ void residual_backward_device(float *g_inp1, float *g_inp2, const float *g_out, int N,
                                         int blockIdx_x);

#endif /* GPT2_LAYERS_RESIDUAL_H */
