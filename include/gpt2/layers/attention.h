...
// Forward pass: 2D grid (B*NH, T), block handles one query position
// Each block computes attention for one (batch, head, query_time)
#define ATTN_FWD_GRID(B, T, NH) dim3((B) * (NH), (T))
#define ATTN_FWD_BLOCK(hs) dim3(ATTN_WARP_SIZE, (hs) / ATTN_WARP_SIZE > 0 ? (hs) / ATTN_WARP_SIZE : 1)
...
#define ATTN_BWD_BLOCK(hs) dim3(ATTN_WARP_SIZE, (hs) / ATTN_WARP_SIZE > 0 ? (hs) / ATTN_WARP_SIZE : 1)
