...
__device__ void mlp_backward_input_device(
    ...
    if (out_row_base + 0 < rows_total) {
        int r0 = out_row_base + 0;
        if (out_col_base + 0 < input_dim) g_input[r0 * input_dim + (out_col_base + 0)] = acc00;
        if (out_col_base + 1 < input_dim) g_input[r0 * input_dim + (out_col_base + 1)] = acc01;
    }
    if (out_row_base + 1 < rows_total) {
        int r1 = out_row_base + 1;
        if (out_col_base + 0 < input_dim) g_input[r1 * input_dim + (out_col_base + 0)] = acc10;
        if (out_col_base + 1 < input_dim) g_input[r1 * input_dim + (out_col_base + 1)] = acc11;
    }
}
...
__device__ void mlp_backward_weight_device(
    ...
    if (out_row_base + 0 < input_dim) {
        int r0 = out_row_base + 0;
        if (out_col_base + 0 < output_dim) g_weight[r0 * output_dim + (out_col_base + 0)] = acc00;
        if (out_col_base + 1 < output_dim) g_weight[r0 * output_dim + (out_col_base + 1)] = acc01;
    }
    if (out_row_base + 1 < input_dim) {
        int r1 = out_row_base + 1;
        if (out_col_base + 0 < output_dim) g_weight[r1 * output_dim + (out_col_base + 0)] = acc10;
        if (out_col_base + 1 < output_dim) g_weight[r1 * output_dim + (out_col_base + 1)] = acc11;
    }
