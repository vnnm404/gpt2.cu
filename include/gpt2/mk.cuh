...
#include "gpt2/device/attention.cuh"
#include "gpt2/device/cross_entropy.cuh"
#include "gpt2/device/embedding.cuh"
#include "gpt2/device/gelu.cuh"
#include "gpt2/device/layernorm.cuh"
#include "gpt2/device/mlp.cuh"
#include "gpt2/device/residual.cuh"
#include "gpt2/device/softmax.cuh"
// #include "gpt2/device/adamw.cuh"
...
#define MK_BLOCK_PARAMS_SIZE                                                   \
  (MK_LN_W_SIZE + MK_LN_B_SIZE + MK_QKV_W_SIZE + MK_QKV_B_SIZE +               \
   MK_ATTN_PROJ_W_SIZE + MK_ATTN_PROJ_B_SIZE + MK_LN_W_SIZE + MK_LN_B_SIZE +   \
   MK_MLP_FC_W_SIZE + MK_MLP_FC_B_SIZE + MK_MLP_PROJ_W_SIZE +                  \
   MK_MLP_PROJ_B_SIZE)
...
#define MK_BLOCK_OFFSET(layer)                                                 \
  (MK_BLOCKS_OFFSET + (layer) * MK_BLOCK_PARAMS_SIZE)
#define MK_LN1_W(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_LN1_W_OFF)
#define MK_LN1_B(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_LN1_B_OFF)
#define MK_QKV_W(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_QKV_W_OFF)
#define MK_QKV_B(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_QKV_B_OFF)
#define MK_ATTN_PROJ_W(base, layer)                                            \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_ATTN_PROJ_W_OFF)
#define MK_ATTN_PROJ_B(base, layer)                                            \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_ATTN_PROJ_B_OFF)
#define MK_LN2_W(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_LN2_W_OFF)
#define MK_LN2_B(base, layer)                                                  \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_LN2_B_OFF)
#define MK_MLP_FC_W(base, layer)                                               \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_MLP_FC_W_OFF)
#define MK_MLP_FC_B(base, layer)                                               \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_MLP_FC_B_OFF)
#define MK_MLP_PROJ_W(base, layer)                                             \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_MLP_PROJ_W_OFF)
#define MK_MLP_PROJ_B(base, layer)                                             \
  ((base) + MK_BLOCK_OFFSET(layer) + MK_BLK_MLP_PROJ_B_OFF)
...
#define MK_ACT_BLOCK_SIZE                                                      \
  (MK_ACT_LN1_SIZE + MK_ACT_LN1_MEAN_SIZE + MK_ACT_LN1_RSTD_SIZE +             \
   ...
   MK_ACT_ATT_PROJ_SIZE + MK_ACT_RES2_SIZE + MK_ACT_LN2_SIZE +                 \
   MK_ACT_LN2_MEAN_SIZE + MK_ACT_LN2_RSTD_SIZE + MK_ACT_MLP_FC_SIZE +          \
   MK_ACT_MLP_FC_GELU_SIZE + MK_ACT_MLP_PROJ_SIZE + MK_ACT_RES3_SIZE)
...
#define MK_ACT_BLK_MLP_PROJ_OFF                                                \
  (MK_ACT_BLK_MLP_FC_GELU_OFF + MK_ACT_MLP_FC_GELU_SIZE)
#define MK_ACT_BLK_RES3_OFF (MK_ACT_BLK_MLP_PROJ_OFF + MK_ACT_MLP_PROJ_SIZE)

// Final layer activations
#define MK_ACT_LN_F_OFFSET                                                     \
  (MK_ACT_BLOCKS_OFFSET + MK_N_LAYER * MK_ACT_BLOCK_SIZE)
...
#define MK_ACT_LN_F_RSTD_OFFSET                                                \
  (MK_ACT_LN_F_MEAN_OFFSET + MK_ACT_LN_F_MEAN_SIZE)
...
#define MK_ACT_BLOCK_OFFSET(layer)                                             \
  (MK_ACT_BLOCKS_OFFSET + (layer) * MK_ACT_BLOCK_SIZE)
#define MK_ACT_ENCODED(base) ((base) + MK_ACT_ENCODED_OFFSET)
#define MK_ACT_LN1(base, layer)                                                \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN1_OFF)
#define MK_ACT_LN1_MEAN(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN1_MEAN_OFF)
#define MK_ACT_LN1_RSTD(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN1_RSTD_OFF)
#define MK_ACT_QKV(base, layer)                                                \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_QKV_OFF)
#define MK_ACT_ATTY(base, layer)                                               \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_ATTY_OFF)
#define MK_ACT_PREATT(base, layer)                                             \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_PREATT_OFF)
#define MK_ACT_ATT(base, layer)                                                \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_ATT_OFF)
#define MK_ACT_ATT_PROJ(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_ATT_PROJ_OFF)
#define MK_ACT_RES2(base, layer)                                               \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_RES2_OFF)
#define MK_ACT_LN2(base, layer)                                                \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN2_OFF)
#define MK_ACT_LN2_MEAN(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN2_MEAN_OFF)
#define MK_ACT_LN2_RSTD(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_LN2_RSTD_OFF)
#define MK_ACT_MLP_FC(base, layer)                                             \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_MLP_FC_OFF)
#define MK_ACT_MLP_FC_GELU(base, layer)                                        \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_MLP_FC_GELU_OFF)
#define MK_ACT_MLP_PROJ(base, layer)                                           \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_MLP_PROJ_OFF)
#define MK_ACT_RES3(base, layer)                                               \
  ((base) + MK_ACT_BLOCK_OFFSET(layer) + MK_ACT_BLK_RES3_OFF)
...
void gpt2_update(gpt2_t *model, gpt2_t *grads, adamw_state_t *opt) {
  ...
  adamw_kernel<<<num_blocks, thr>>>(
      model->params_memory, grads->params_memory, opt->m_memory, opt->v_memory,
      model->num_parameters, opt->learning_rate, opt->beta1, opt->beta2,
      opt->eps, opt->weight_decay, opt->t);

  gpuErrchk(cudaGetLastError());
}

void add_instructions_1d(instruction_t *all_instructions,
                         int *instruction_count, int op, int prev_op, int layer,
                         int bar_idx, int expected, int num_blocks) {
  int num_instructions = (num_blocks <= NUM_SM) ? num_blocks : NUM_SM;
  int blocks_per_instruction =
      (num_blocks + num_instructions - 1) / num_instructions;

  for (int i = 0; i < num_instructions; i++) {
    int start_b_x = i * blocks_per_instruction;
    int end_b_x = ((i + 1) * blocks_per_instruction < num_blocks)
                      ? ((i + 1) * blocks_per_instruction - 1)
                      : (num_blocks - 1);

    if (start_b_x < num_blocks) {
      all_instructions[(*instruction_count)++] =
          (instruction_t){.op = op,
                          .prev_op = prev_op,
                          .layer = layer,
                          .start_b_x = start_b_x,
...
void add_instructions_2d(instruction_t *all_instructions,
                         int *instruction_count, int op, int prev_op, int layer,
                         int bar_idx, int expected, int num_blocks_x,
                         int num_blocks_y) {
  int total_blocks = num_blocks_x * num_blocks_y;
  int num_instructions = (total_blocks <= NUM_SM) ? total_blocks : NUM_SM;
  int blocks_per_instruction =
      (total_blocks + num_instructions - 1) / num_instructions;
...
stream_t **schedule_instructions(config_t config, stream_t **streams,
                                 int seq_len) {
  ...
  instruction_t *all_instructions =
      (instruction_t *)malloc(max_instructions * sizeof(instruction_t));
  ...
  // OP 1: Embedding forward - [B blocks, 1D grid]
  {
    ...
    add_instructions_1d(all_instructions, &instruction_count, op, prev_op,
                        layer, bar_idx, expected, num_blocks);
    prev_op = op;
  }
  ...
  for (int layer_idx = 0; layer_idx < L; layer_idx++) {
    // OP 2: LayerNorm 1 - [B blocks, 1D grid]
    {
      ...
      add_instructions_1d(all_instructions, &instruction_count, op, prev_op,
                          layer_idx, bar_idx, expected, num_blocks);
      prev_op = op;
    }
    ...
    // OP 3: QKV - [MLP_FORWARD_GRID(h * 3, B, S) blocks, 2D grid]
    {
      ...
      add_instructions_2d(all_instructions, &instruction_count, op, prev_op,
                          layer_idx, bar_idx, expected, num_blocks_x,
                          num_blocks_y);
      prev_op = op;
    }
  ...
  // OP 12: Final LayerNorm - [B blocks, 1D grid]
  {
    ...
    add_instructions_1d(all_instructions, &instruction_count, op, prev_op, -1,
                        bar_idx, expected, num_blocks);
    prev_op = op;
  }
  ...
  // OP 13: Logits - [MLP_FORWARD_GRID(V, B, S) blocks, 2D grid]
  {
    ...
    add_instructions_2d(all_instructions, &instruction_count, op, prev_op, -1,
                        bar_idx, expected, num_blocks_x, num_blocks_y);
    prev_op = op;
  }
  ...
  // OP 18: Embedding weight gradient - [MLP_BACKWARD_WEIGHT_GRID(V, h) blocks,
  // 2D grid]
  {
    int op = 18;
  ...
  for (int layer_idx = L - 1; layer_idx >= 0; layer_idx--) {
    // OP 20: Residual backward (res_3) - [CEIL_DIV(B * S * h, thr) blocks, 1D
    // grid]
    {
      int op = 20;
      int bar_idx =
          (layer_idx == L - 1)
              ? (1 + (L * 10) + 3 + 4)
              : (1 + (L * 10) + 3 + 5 + ((L - 1 - layer_idx) - 1) * 14 + 13);
      int expected = B;
      int num_blocks = CEIL_DIV(B * S * h, thr);
    ...
    // OP 21: MLP projection backward input - [MLP_BACKWARD_INPUT_GRID(h * 4, B,
    // S) blocks, 2D grid]
    {
      int op = 21;
    ...
    // OP 22: MLP projection backward weight - [MLP_BACKWARD_WEIGHT_GRID(h, h *
    // 4) blocks, 2D grid]
    {
      int op = 22;
    ...
    // OP 24: MLP FC backward input - [MLP_BACKWARD_INPUT_GRID(h, B, S) blocks,
    // 2D grid]
    {
      int op = 24;
    ...
    // OP 25: MLP FC backward weight - [MLP_BACKWARD_WEIGHT_GRID(h * 4, h)
    // blocks, 2D grid]
    {
      int op = 25;
    ...
    // OP 27: Residual backward (res_2) - [CEIL_DIV(B * S * h, thr) blocks, 1D
    // grid]
    {
      int op = 27;
    ...
    // OP 28: Attention projection backward input - [MLP_BACKWARD_INPUT_GRID(h,
    // B, S) blocks, 2D grid]
    {
      int op = 28;
    ...
    // [MLP_BACKWARD_WEIGHT_GRID(h, h) blocks, 2D grid]
    {
      int op = 29;
    ...
    // OP 30: Attention backward - [CEIL_DIV(B * S * n_head, thr) blocks, 1D
    // grid]
    {
      int op = 30;
    ...
    // OP 31: QKV backward input - [MLP_BACKWARD_INPUT_GRID(h, B, S) blocks, 2D
    // grid]
    {
      int op = 31;
    ...
    // OP 32: QKV backward weight - [MLP_BACKWARD_WEIGHT_GRID(h * 3, h) blocks,
    // 2D grid]
    {
      int op = 32;
  ...
  for (int sm = 0; sm < NUM_SM; sm++) {
    if (sm_counts[sm] > 0) {
      ...
      st->n = sm_counts[sm];
      st->instructions =
          (instruction_t *)malloc(sm_counts[sm] * sizeof(instruction_t));
  ...
  for (int sm = 0; sm < NUM_SM; sm++) {
    ...
    if (host_stream_structs[sm] && host_stream_structs[sm]->n > 0) {
      gpuErrchk(cudaMalloc(&d_instructions,
                           host_stream_structs[sm]->n * sizeof(instruction_t)));
      gpuErrchk(cudaMemcpy(d_instructions,
                           host_stream_structs[sm]->instructions,
                           host_stream_structs[sm]->n * sizeof(instruction_t),
                           cudaMemcpyHostToDevice));
    }

    stream_t temp_stream = {host_stream_structs[sm] ? host_stream_structs[sm]->n
                                                    : 0,
                            d_instructions};
    gpuErrchk(cudaMemcpy(d_stream, &temp_stream, sizeof(stream_t),
                         cudaMemcpyHostToDevice));
  ...
  gpuErrchk(cudaMemcpy(d_streams_ptr, d_stream_structs,
                       NUM_SM * sizeof(stream_t *), cudaMemcpyHostToDevice));
...
void free_schedule(stream_t **d_streams_ptr) {
  ...
  stream_t *d_stream_structs[NUM_SM];
  gpuErrchk(cudaMemcpy(d_stream_structs, d_streams_ptr,
                       NUM_SM * sizeof(stream_t *), cudaMemcpyDeviceToHost));

  for (int sm = 0; sm < NUM_SM; sm++) {
    stream_t temp_stream;
    gpuErrchk(cudaMemcpy(&temp_stream, d_stream_structs[sm], sizeof(stream_t),
...
__device__ void execute_instruction(float *params, float *grads,

                                    float *acts, float *grad_acts,

                                    int seq_len, const int *d_input_tokens,
  ...
  case 1: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      embedding_forward_device(MK_ACT_ENCODED(acts), d_input_tokens,
                               MK_WTE(params), MK_WPE(params), S, h, V,
                               MK_N_POSITIONS, b_x);
    }
    break;
  }
  ...
  case 2: {
    // OP 2: LayerNorm 1
    float *res = (instr.layer == 0) ? MK_ACT_ENCODED(acts)
                                    : MK_ACT_RES3(acts, instr.layer - 1);
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_forward_device(
          MK_ACT_LN1(acts, instr.layer), res, MK_LN1_W(params, instr.layer),
          MK_LN1_B(params, instr.layer), MK_ACT_LN1_MEAN(acts, instr.layer),
          MK_ACT_LN1_RSTD(acts, instr.layer), S, h, b_x);
    }
    break;
  }
  ...
  case 3: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
         linear_idx++) {
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_forward_device(
          MK_ACT_QKV(acts, instr.layer), MK_ACT_LN1(acts, instr.layer),
          MK_QKV_W(params, instr.layer), MK_QKV_B(params, instr.layer), B, S, h,
          h * 3, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 4: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      attention_forward_device(
          MK_ACT_ATTY(acts, instr.layer), MK_ACT_PREATT(acts, instr.layer),
          MK_ACT_ATT(acts, instr.layer), MK_ACT_QKV(acts, instr.layer), B, S,
          n_head, h, b_x);
    }
    break;
  }
  ...
  case 5: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_forward_device(MK_ACT_ATT_PROJ(acts, instr.layer),
                         MK_ACT_ATTY(acts, instr.layer),
                         MK_ATTN_PROJ_W(params, instr.layer),
                         MK_ATTN_PROJ_B(params, instr.layer), B, S, h, h, b_x,
                         b_y, shared_mem);
    }
    break;
  }
  ...
  case 6: {
    ...
    float *res = (instr.layer == 0) ? MK_ACT_ENCODED(acts)
                                    : MK_ACT_RES3(acts, instr.layer - 1);
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      residual_forward_device(MK_ACT_RES2(acts, instr.layer),
                              MK_ACT_ATT_PROJ(acts, instr.layer), res, B, S, h,
                              b_x);
    }
    break;
  }
  ...
  case 7: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_forward_device(
          MK_ACT_LN2(acts, instr.layer), MK_ACT_RES2(acts, instr.layer),
          MK_LN2_W(params, instr.layer), MK_LN2_B(params, instr.layer),
          MK_ACT_LN2_MEAN(acts, instr.layer),
          MK_ACT_LN2_RSTD(acts, instr.layer), S, h, b_x);
    }
    break;
  }
  ...
  case 8: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_x = linear_idx % num_blocks_x;
      mlp_forward_device(
          MK_ACT_MLP_FC(acts, instr.layer), MK_ACT_LN2(acts, instr.layer),
          MK_MLP_FC_W(params, instr.layer), MK_MLP_FC_B(params, instr.layer), B,
          S, h, h * 4, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 9: {
    // OP 9: GELU
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      gelu_forward_device(MK_ACT_MLP_FC_GELU(acts, instr.layer),
                          MK_ACT_MLP_FC(acts, instr.layer), B, S, h * 4, b_x);
    }
    break;
  }
  ...
  case 10: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_forward_device(MK_ACT_MLP_PROJ(acts, instr.layer),
                         MK_ACT_MLP_FC_GELU(acts, instr.layer),
                         MK_MLP_PROJ_W(params, instr.layer),
                         MK_MLP_PROJ_B(params, instr.layer), B, S, h * 4, h,
                         b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 11: {
    // OP 11: Residual 3
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      residual_forward_device(MK_ACT_RES3(acts, instr.layer),
                              MK_ACT_MLP_PROJ(acts, instr.layer),
                              MK_ACT_RES2(acts, instr.layer), B, S, h, b_x);
    }
    break;
  }
  ...
  case 12: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_forward_device(MK_ACT_LN_F(acts), MK_ACT_RES3(acts, L - 1),
                               MK_LN_F_W(params), MK_LN_F_B(params),
                               MK_ACT_LN_F_MEAN(acts), MK_ACT_LN_F_RSTD(acts),
                               S, h, b_x);
    }
    break;
  }
  ...
  case 13: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_forward_device(MK_ACT_LOGITS(acts), MK_ACT_LN_F(acts), MK_WTE(params),
                         NULL, B, S, h, V, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 14: {
    // OP 14: Softmax
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      softmax_forward_device(MK_ACT_PROBS(acts), MK_ACT_LOGITS(acts), B, S, V,
                             b_x, shared_mem);
    }
    break;
  }
  ...
  case 15: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      cross_entropy_forward_device(MK_ACT_LOSSES(acts), MK_ACT_PROBS(acts),
                                   d_target_tokens, B, S, V, b_x);
    }
    break;
  }
  ...
  case 16: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      cross_entropy_backward_device(MK_ACT_LOGITS(grad_acts),
                                    MK_ACT_PROBS(acts), d_target_tokens, B, S,
                                    V, b_x);
    }
    break;
  }
  ...
  case 17: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_input_device(MK_ACT_LN_F(grad_acts),
                                MK_ACT_LOGITS(grad_acts), MK_WTE(params), B, S,
                                h, V, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 18: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_weight_device(MK_WTE(grads), NULL, MK_ACT_LOGITS(grad_acts),
                                 MK_ACT_LN_F(acts), B, S, h, V, b_x, b_y,
                                 shared_mem);
    }
    break;
  }
  ...
  case 19: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_backward_device(
          MK_ACT_RES3(grad_acts, L - 1), MK_LN_F_W(grads), MK_LN_F_B(grads),
          MK_ACT_LN_F(grad_acts), MK_ACT_RES3(acts, L - 1), MK_LN_F_W(params),
          MK_ACT_LN_F_MEAN(acts), MK_ACT_LN_F_RSTD(acts), B, S, h, b_x);
    }
    break;
  }
  ...
  case 20: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      residual_backward_device(MK_ACT_RES2(grad_acts, instr.layer),
                               MK_ACT_MLP_PROJ(grad_acts, instr.layer),
                               MK_ACT_RES3(grad_acts, instr.layer), B * S * h,
                               b_x);
    }
    break;
  }
  ...
  case 21: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_input_device(MK_ACT_MLP_FC_GELU(grad_acts, instr.layer),
                                MK_ACT_MLP_PROJ(grad_acts, instr.layer),
                                MK_MLP_PROJ_W(params, instr.layer), B, S, h * 4,
                                h, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 22: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_weight_device(MK_MLP_PROJ_W(grads, instr.layer),
                                 MK_MLP_PROJ_B(grads, instr.layer),
                                 MK_ACT_MLP_PROJ(grad_acts, instr.layer),
                                 MK_ACT_MLP_FC_GELU(acts, instr.layer), B, S,
                                 h * 4, h, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 23: {
    // OP 23: GELU backward
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      gelu_backward_device(MK_ACT_MLP_FC(grad_acts, instr.layer),
                           MK_ACT_MLP_FC(acts, instr.layer),
                           MK_ACT_MLP_FC_GELU(grad_acts, instr.layer),
                           B * S * 4 * h, b_x);
    }
    break;
  }
  ...
  case 24: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_input_device(MK_ACT_LN2(grad_acts, instr.layer),
                                MK_ACT_MLP_FC(grad_acts, instr.layer),
                                MK_MLP_FC_W(params, instr.layer), B, S, h,
                                h * 4, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 25: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      mlp_backward_weight_device(
          MK_MLP_FC_W(grads, instr.layer), MK_MLP_FC_B(grads, instr.layer),
          MK_ACT_MLP_FC(grad_acts, instr.layer), MK_ACT_LN2(acts, instr.layer),
          B, S, h, h * 4, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 26: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_backward_device(
          MK_ACT_RES2(grad_acts, instr.layer), MK_LN2_W(grads, instr.layer),
          MK_LN2_B(grads, instr.layer), MK_ACT_LN2(grad_acts, instr.layer),
          MK_ACT_RES2(acts, instr.layer), MK_LN2_W(params, instr.layer),
          MK_ACT_LN2_MEAN(acts, instr.layer),
          MK_ACT_LN2_RSTD(acts, instr.layer), B, S, h, b_x);
    }
    break;
  }
  ...
  case 27: {
    ...
    float *g_res = (instr.layer == 0) ? MK_ACT_ENCODED(grad_acts)
                                      : MK_ACT_RES3(grad_acts, instr.layer - 1);
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      residual_backward_device(g_res, MK_ACT_ATT_PROJ(grad_acts, instr.layer),
                               MK_ACT_RES2(grad_acts, instr.layer), B * S * h,
                               b_x);
    }
    break;
  }
  ...
  case 28: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_input_device(MK_ACT_ATTY(grad_acts, instr.layer),
                                MK_ACT_ATT_PROJ(grad_acts, instr.layer),
                                MK_ATTN_PROJ_W(params, instr.layer), B, S, h, h,
                                b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 29: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_weight_device(MK_ATTN_PROJ_W(grads, instr.layer),
                                 MK_ATTN_PROJ_B(grads, instr.layer),
                                 MK_ACT_ATT_PROJ(grad_acts, instr.layer),
                                 MK_ACT_ATTY(acts, instr.layer), B, S, h, h,
                                 b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 30: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      attention_backward_device(
          MK_ACT_QKV(grad_acts, instr.layer),
          MK_ACT_PREATT(grad_acts, instr.layer),
          MK_ACT_ATT(grad_acts, instr.layer),
          MK_ACT_ATTY(grad_acts, instr.layer), MK_ACT_QKV(acts, instr.layer),
          MK_ACT_ATT(acts, instr.layer), B, S, h, n_head, b_x);
    }
    break;
  }
  ...
  case 31: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_y = linear_idx / num_blocks_x;
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_input_device(MK_ACT_LN1(grad_acts, instr.layer),
                                MK_ACT_QKV(grad_acts, instr.layer),
                                MK_QKV_W(params, instr.layer), B, S, h, h * 3,
                                b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 32: {
    ...
    for (int linear_idx = start_linear; linear_idx <= end_linear;
      ...
      int b_x = linear_idx % num_blocks_x;
      mlp_backward_weight_device(
          MK_QKV_W(grads, instr.layer), MK_QKV_B(grads, instr.layer),
          MK_ACT_QKV(grad_acts, instr.layer), MK_ACT_LN1(acts, instr.layer), B,
          S, h, h * 3, b_x, b_y, shared_mem);
    }
    break;
  }
  ...
  case 33: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      layernorm_backward_device(
          g_res, MK_LN1_W(grads, instr.layer), MK_LN1_B(grads, instr.layer),
          MK_ACT_LN1(grad_acts, instr.layer), res,
          MK_LN1_W(params, instr.layer), MK_ACT_LN1_MEAN(acts, instr.layer),
          MK_ACT_LN1_RSTD(acts, instr.layer), B, S, h, b_x);
    }
    break;
  }
  ...
  case 34: {
    ...
    for (int b_x = start_b_x; b_x <= end_b_x; b_x++) {
      embedding_backward_device(MK_WTE(grads), MK_WPE(grads),
                                MK_ACT_ENCODED(grad_acts), d_input_tokens, B, S,
                                h, b_x);
    }
    break;
  }
...
__device__ void execute_stream(float *params, float *grads,
                               ...
                               int *bar, stream_t *stream) {
  int n = stream->n;
  for (int i = 0; i < n; i++) {
    instruction_t instr = stream->instructions[i];
    execute_instruction(params, grads, acts, grad_acts, seq_len, d_input_tokens,
                        d_target_tokens,
#ifdef PROFILE
...
__global__ void megakernel(float *params, float *grads,
...
#ifdef PROFILE
                           long long *sm_start_times, long long *sm_end_times,
                           long long *bar_enter_time, long long *bar_exit_time,
                           long long *instr_end_time,
#endif

                           int *bar, stream_t **streams) {
  int sm_id = blockIdx.x;
  ...
  execute_stream(params, grads, acts, grad_acts, seq_len, d_input_tokens,
                 d_target_tokens,
#ifdef PROFILE
                 bar_enter_time, bar_exit_time, instr_end_time,
#endif
