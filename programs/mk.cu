...
void add_instructions_1d(instruction_t *all_instructions, int *instruction_count, 
    ...
    for (int i = 0; i < num_instructions; i++) {
        ...
        int start_b_x = sum;
        int end_b_x = (sum + blocks_per_instruction < num_blocks) ? 
                      (sum + blocks_per_instruction - 1) : (num_blocks - 1);
...
void add_instructions_2d(instruction_t *all_instructions, int *instruction_count,
    ...
    for (int i = 0; i < num_instructions; i++) {
        ...
        int end_linear = (sum + blocks_per_instruction < total_blocks) ?
                         (sum + blocks_per_instruction - 1) : (total_blocks - 1);
        sum += blocks_per_instruction;
