...
// Device function for attention forward pass - Megakernel compatible
...
__global__ void attention_forward(float *out, float *preatt, float *att,
                                  ...
                                  int B, int T, int NH, int C)
{
    attention_forward_device(out, preatt, att, inp, B, T, NH, C, blockIdx.x);
}

// CUDA kernel for attention backward pass
// Each thread handles one (batch, time, head) position
// g_inp: [B, T, 3*C] gradients for Q,K,V
// g_preatt: [B, NH, T, T] gradients for pre-attention scores
// g_att: [B, NH, T, T] gradients for attention weights
...
__global__ void attention_backward(float *g_inp, float *g_preatt, float *g_att,
                                   ...
                                   int B, int T, int C, int NH)
{
    attention_backward_device(g_inp, g_preatt, g_att, g_out, inp, att, B, T, C, NH, blockIdx.x);
}
