...
void forward(config_t config, gpt2_t model, train_buffers_t buffers, const int *d_input_tokens, int seq_len)
{
    ...
    for (int layer_idx = 0; layer_idx < config.n_layer; layer_idx++)
    {
        ...
        attention_forward<<<ATTN_FWD_GRID(B, S, n_head), ATTN_FWD_BLOCK(), ATTN_FWD_SMEM(S, h / n_head)>>>(layer_bufs->atty.data, layer_bufs->preatt.data, layer_bufs->att.data, layer_bufs->qkv.data, B, S, n_head, h);
...
void backward(config_t config, gpt2_t model, train_buffers_t buffers, gpt2_t g_model, train_buffers_t g_buffers, const int *d_input_tokens, const int *d_target_tokens, int seq_len)
{
    ...
    for (int layer_idx = L - 1; layer_idx >= 0; layer_idx--)
    {
        ...
        attention_backward<<<ATTN_BWD_GRID(B, S, n_head), ATTN_BWD_BLOCK(), ATTN_BWD_SMEM(S, h / n_head)>>>(g_layer_bufs->qkv.data, g_layer_bufs->preatt.data, g_layer_bufs->att.data, g_layer_bufs->atty.data, layer_bufs->qkv.data, layer_bufs->att.data, B, S, h, n_head);
