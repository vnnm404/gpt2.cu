...
int main(int argc, char *argv[])
{
    ...
    FILE *state_file = fopen("../models/gpt2_multibatch_state.bin", "rb");
    if (!state_file) {
        fprintf(stderr, "Error opening state file\n");
        return 1;
    }
    ...
    int header_raw[256];
    fread(header_raw, sizeof(int), 256, state_file);

    StateHeader header;
    header.magic          = header_raw[0];
    header.version        = header_raw[1];
    header.num_batches    = header_raw[2];
    header.B              = header_raw[3];
    header.T              = header_raw[4];
    header.V              = header_raw[5];
    header.num_parameters = header_raw[6];
    header.flags          = header_raw[7];


    if (header.magic != 20240327 || header.version != 2) {
    ...
    printf("Batches: %d\n", header.num_batches);
    printf("B=%d T=%d V=%d\n", header.B, header.T, header.V);
    int *x[header.num_batches];
    int *y[header.num_batches];
    for(int b = 0; b < header.num_batches; b++){
        x[b] = (int *)malloc(header.B * header.T * sizeof(int));
        y[b] = (int *)malloc(header.B * header.T * sizeof(int));
    }

    float *logits = NULL;
    float *grads  = NULL;
    float loss[header.num_batches];
    printf("headerflag %d\n", header.flags);
    if (header.flags & (1 << 0))
        logits = (float *)malloc(header.B * header.T * header.V * sizeof(float));

    if (header.flags & (1 << 2))
        grads = (float *)malloc(header.num_parameters * sizeof(float));

    for (int b = 0; b < header.num_batches; b++) {
        fread(x[b], sizeof(int), header.B * header.T, state_file);
        fread(y[b], sizeof(int), header.B * header.T, state_file);

        if (header.flags & (1 << 0))
            fread(logits, sizeof(float), header.B * header.T * header.V, state_file);

        if (header.flags & (1 << 1))
            fread(&loss[b], sizeof(float), 1, state_file);

        if (header.flags & (1 << 2))
            fread(grads, sizeof(float), header.num_parameters, state_file);

        // ---- Use batch b here ----
        // printf("Loaded batch %d, loss=%f\n", b, loss[b]);
    }
    ...
    gpuErrchk(cudaMemcpy(d_input_tokens, x[0], B * T * sizeof(int), cudaMemcpyHostToDevice));
    gpuErrchk(cudaMemcpy(d_target_tokens, y[0], B * T * sizeof(int), cudaMemcpyHostToDevice));
    ...
    int num_steps = 1;
    float losses[header.num_batches * num_steps];

    // Run 10 training iterations
    // MARK: ITER
    for (int step = 0; step < num_steps; step++)
    {
        for(int b = 0; b < min(10, header.num_batches); b++)
        {   
            ...
            gpuErrchk(cudaMemcpy(d_input_tokens, x[b], B * T * sizeof(int), cudaMemcpyHostToDevice));
            gpuErrchk(cudaMemcpy(d_target_tokens, y[b], B * T * sizeof(int), cudaMemcpyHostToDevice));
            

            // Zero gradients
            gpt2_zero_grad(&g_model);
            zero_activation_grads(config, &g_buffers);
            // zero_activation_grads(config, &buffers);
    ...
    FILE *checkpoint_file = fopen("../models/checkpoint.bin", "wb");
    int ret = gpt2_dump_weights(&model, checkpoint_file);
    printf("ret[%d]\n", ret);
