...
int main(int argc, char *argv[]) {
  ...
  printf("GPU: "
         ...
         p.name, p.maxThreadsPerBlock, p.sharedMemPerBlock, p.regsPerBlock,
  ...
  // allocate global bar
  int bar_size = config.batch_size * (1 + (config.n_layer * 10 + 3) + 1 +
                                      (5 + config.n_layer * 14 + 1));
  ...
  // Allocate per-instruction timing buffers for bar enter/exit times and
  // instruction end times
  gpuErrchk(cudaMalloc(&d_bar_enter_time,
                       NUM_SM * MAX_INSTR_PER_SM * sizeof(long long)));
  gpuErrchk(cudaMalloc(&d_bar_exit_time,
                       NUM_SM * MAX_INSTR_PER_SM * sizeof(long long)));
  gpuErrchk(cudaMalloc(&d_instr_end_time,
                       NUM_SM * MAX_INSTR_PER_SM * sizeof(long long)));
  long long *h_bar_enter_time =
      (long long *)malloc(NUM_SM * MAX_INSTR_PER_SM * sizeof(long long));
  long long *h_bar_exit_time =
      (long long *)malloc(NUM_SM * MAX_INSTR_PER_SM * sizeof(long long));
  long long *h_instr_end_time =
      (long long *)malloc(NUM_SM * MAX_INSTR_PER_SM * sizeof(long long));
  ...
  for (int step = 0; step < 10; step++) {
    ...
    megakernel<<<NUM_SM, threads_per_block, shared_mem_size>>>(
        model.params_memory, g_model.params_memory, buffers.activations_memory,
        g_buffers.activations_memory, T, d_input_tokens, d_target_tokens,

#ifdef PROFILE
        d_sm_start_times, d_sm_end_times, d_bar_enter_time, d_bar_exit_time,
        d_instr_end_time,
#endif
    ...
    gpuErrchk(cudaMemcpy(h_sm_start_times, d_sm_start_times,
                         NUM_SM * sizeof(long long), cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(h_sm_end_times, d_sm_end_times,
                         NUM_SM * sizeof(long long), cudaMemcpyDeviceToHost));
    ...
    gpuErrchk(cudaMemcpy(h_bar_enter_time, d_bar_enter_time,
                         NUM_SM * MAX_INSTR_PER_SM * sizeof(long long),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(h_bar_exit_time, d_bar_exit_time,
                         NUM_SM * MAX_INSTR_PER_SM * sizeof(long long),
                         cudaMemcpyDeviceToHost));
    gpuErrchk(cudaMemcpy(h_instr_end_time, d_instr_end_time,
                         NUM_SM * MAX_INSTR_PER_SM * sizeof(long long),
                         cudaMemcpyDeviceToHost));
      ...
      if (f) {
        ...
        for (int sm = 0; sm < NUM_SM; sm++) {
          ...
          for (int i = 0; i < h_instr_counts[sm]; i++) {
            ...
            fprintf(f,
                    "  instr %d: bar_enter=%lld, bar_exit=%lld, "
                    "instr_end=%lld, spin_wait=%lld, exec_time=%lld\n",
                    i, enter - min_start, exit - min_start, end - min_start,
                    spin_duration / 1000000, exec_duration / 1000000);
          }
        }
        fprintf(f, "\n");
        fclose(f);
